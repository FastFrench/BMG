	...
	public abstract class OpenGLObject : IDisposable
	{
		...
		virtual public void OnUpdateObject(FrameEventArgs e)
		{
			...
			if (error != ErrorCode.NoError)
				Debug.Print("OpenGL error binding ElevationBuffer: " + error.ToString());
			// Update normals
			if (Normals != null)
			{
			...
			error = GL.GetError();
			if (error != ErrorCode.NoError)
				Debug.Print("OpenGL error binding NormalBuffer: " + error.ToString());
		...
		public void LoadShaders(byte[] pixelShaderData, byte[] vertexShaderData, byte[] geometryShaderData = null)
		{
			...
			var error = GL.GetError();
			if (error != ErrorCode.NoError)
				Debug.Print("OpenGL error loading Shaders: " + error.ToString());
